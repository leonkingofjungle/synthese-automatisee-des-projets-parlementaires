...
# automated-parliamentary-bill-summaries (EN)

Plain-language summaries of the latest French parliamentary bills, generated by AI
from official source texts and published on a static website.
...
The reliability score is computed by a model that is independent from the one
that generates the summary (extractive verification: every statement in the
summary must be supported by the source document). If the score is below 60/100,
the summary is not published (up to 3 attempts are made before the document is
discarded). If the score is below 85/100 or an anomaly is detected, the website
displays a caution badge. Without LM Studio, everything still works: summaries
are simply published without a reliability score.

## Requirements

* Python 3.11+ and `pip install -r requirements.txt`
* A Gemini API key in the root `.env` file: `GEMINI_API_KEY=...` (see `.env.example`)
* Optional — the local judge: LM Studio running in server mode with a loaded
  model (default: `mistral-nemo-instruct-2407`). Optional `.env` variables:
  `JUDGE_BASE_URL` (default: `http://localhost:1234/v1`) and
  `JUDGE_MODEL` (the exact model identifier displayed by LM Studio). A context
  window of at least 16k tokens is recommended.

## Generate summaries

```bash
python3 scripts/pdf_summarizer_mcp.py --limit 10                 # latest 10 bills
python3 scripts/pdf_summarizer_mcp.py --type PION --chambre AN   # default filters shown
python3 scripts/pdf_summarizer_mcp.py --force                    # regenerate the current window
python3 scripts/pdf_summarizer_mcp.py --workers 10               # increase parallelism
```

`front/resumes.json` acts as a cache: only documents that are not already
present are generated. `--force` regenerates the documents within the current
`--limit` window; summaries outside that window are always preserved. To
regenerate the entire dataset, either delete `front/resumes.json` or specify a
`--limit` large enough to cover the full corpus.

## Serve the website

Run the server **from the `front/` directory**, never from the project root
(otherwise the server could expose the `.env` file):
